...
output = WORK / "output"
run = subprocess.run([sys.executable, str(ROOT / "implementation/build_delivery.py"), "--input", str(WORK / "input_data"), "--output", str(output)], cwd=ROOT, text=True, capture_output=True, timeout=600)
...
summary = {"result": "PASS", "mode": "reference", "commit_sha": os.getenv("GITHUB_SHA"), "workflow_run_id": os.getenv("GITHUB_RUN_ID"), "reference_members": sorted(path.relative_to(WORK).as_posix() for path in output.rglob("*") if path.is_file() and "__pycache__" not in path.parts)}
